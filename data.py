...
class Dataset_SVC2004_train(Dataset):
    def __init__(self):
        self.traintxtList = mixTxtSameWriter(1, 32)
        # self.txtList = readicdar(icdartraintruepath, icdartrainfalsepath)

    def __getitem__(self, index):
        labelFileName, testFilename, label = self.traintxtList[index]
...
class Dataset_SVC2004_test(Dataset):
    def __init__(self):
        self.testtxtList = mixTxtSameWriter(33, 40)
        # self.txtList = readicdar(icdartraintruepath, icdartrainfalsepath)

    def __getitem__(self, index):
        labelFileName, testFilename, label = self.testtxtList[index]
        # labelPath = os.path.join(rootPath, labelFileName)
